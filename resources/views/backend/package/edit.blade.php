					...
					<div class="col-md-12">
						...
						<table class="table table-bordered">
							<thead class="thead-dark">
							   <th class="w-33">{{ _lang('Monthly Limit') }}</th>
							   <th class="w-33">{{ _lang('6 Months Limit') }}</th>
							   <th class="w-33">{{ _lang('Yearly Limit') }}</th>
							</thead>
							<tbody>
								<tr>
									<td>
									...
									<td>
										<div>
										  <div class="form-group">
											<label class="control-label">{{ _lang('Websites Limit') }}</label>						
											<select class="free form-control select2" name="websites_limit[6_months]" id="websites_limit_6months" required>
												<option value="No">{{ _lang('No') }}</option>
												<option value="Unlimited">{{ _lang('Unlimited') }}</option>
												@for( $i = 1; $i <= 60; $i++ )
													<option value="{{ $i }}">{{ $i }}</option>
												@endfor
												@if ($package->type != 'free')
													<option selected value="{{ unserialize($package->websites_limit)['6_months'] }}">{{ unserialize($package->websites_limit)['6_months'] }}</option>
												@endif 
											</select>
										  </div>
										</div>
								...
								<tr>
									...
									<td>				
										<div>
										  <div class="form-group">
											<label class="control-label">{{ _lang('Recurring Transaction') }}</label>					
											<select class="free form-control select2" name="recurring_transaction[6_months]" id="recurring_transaction_6months" required>
												<option value="Yes">{{ _lang('Yes') }}</option>
												<option value="No">{{ _lang('No') }}</option>
											</select>
										  </div>
								...
								<tr>
									...
									<td>								
										<div>
										  <div class="form-group">
											<label class="control-label">{{ _lang('Online Payment') }}</label>						
											<select class="free form-control select2" name="online_payment[6_months]" id="online_payment_6months" required>
												<option value="No">{{ _lang('No') }}</option>
												<option value="Yes">{{ _lang('Yes') }}</option>
											</select>
										  </div>
								...
								<tr>
									...
									<td>
										<div>
										  <div class="form-group">
											<label class="control-label">{{ _lang('Cost Per 6 Months').' '.currency() }}</label>						
											<input type="text" class="free form-control float-field" name="cost_per_6_months" value="{{ $package->cost_per_6_months }}" required>
										  </div>
										</div>
									</td>
...
(function($) {
	...
	$("#recurring_transaction_6months").val("{{ unserialize($package->recurring_transaction)['6_months'] }}").trigger('change');
	...
	$("#online_payment_6months").val("{{ unserialize($package->online_payment)['6_months'] }}").trigger('change');
